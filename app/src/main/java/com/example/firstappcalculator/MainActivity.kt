...
class MainActivity : AppCompatActivity() {
    ...
    fun initState(){
        ...
        btn_negative.setOnClickListener{
            if (count == 0) {
                val zero = 0
                value = calculator.substract(zero.toString(), ResultText.text.toString())
                ResultText.text = "$value"
                count += 1
            }
            else {
                val zero = 0
                 val res = calculator.substract(zero.toString(), ResultText.text.toString())
                ResultText.text = "$res"
            }
        ...
        btn_equal.setOnClickListener{
            if(operation == 1){
                ...
                val rez = calculator.substract(value,ResultText.text.toString())
                WholeText.text = WholeText.text.toString() +  " = " + ResultText.text.toString()
            }else if(operation == 4){
                WholeText.text = WholeText.text.toString() + ResultText.text.toString()
